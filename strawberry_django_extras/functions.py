...
from django.db.models.fields.related import ForeignKey, ManyToManyField, ManyToManyRel, ManyToOneRel, OneToOneField, OneToOneRel
from strawberry import UNSET
from strawberry.utils.str_converters import to_camel_case
from strawberry_django.utils.inspect import get_model_fields

from .exceptions import SDJExtrasError
from .inputs import CRUDManyToManyCreateInput, CRUDManyToManyUpdateInput, CRUDManyToOneCreateInput, CRUDManyToOneUpdateInput, CRUDOneToManyCreateInput, CRUDOneToManyUpdateInput, CRUDOneToOneCreateInput, CRUDOneToOneUpdateInput


@transaction.atomic
def kill_a_rabbit(data, caller_data, is_before=True, is_root: bool = False, next_=None, source=None, info=None,  # noqa: PLR0912, PLR0913, PLR0915
                  ni=None):
    if data.get("before"):
    ...
    if is_root:
        ...
        if data.get("operation") == "create":
            if data.get("m2m", False) is True:
                ...
                obj = manager.create(**data.get("data"), through_defaults=data.get("through_defaults")) if data.get("through_defaults", None) is not None else manager.create(**data.get("data"))
            else:
            ...
            if isinstance(val, (OneToOneField, OneToOneRel)):
                ...
                if isinstance(_rel_input, CRUDOneToOneUpdateInput):
                    ...
                    if _rel_input.update is not UNSET and (
                        ...
                        raise SDJExtrasError("Updating an object is only supported without create/assign.")
                    if _rel_input.assign is not UNSET:
                        if isinstance(val, OneToOneRel):
                            if val.remote_field.null is False:
                                if _rel_input.delete is not True and existing_instance is not None:
                                    raise SDJExtrasError(
                                        f"There is a {key} already assigned to this {val.remote_field.name} and the field is not nullable. Maybe specify delete to replace the {key} ?")
                            else:  # noqa: PLR5501
                                if _rel_input.delete is not True and existing_instance is not None:
                    ...
                    if _rel_input.create is not UNSET:
                        if isinstance(val, OneToOneRel) and _rel_input.delete is not True and existing_instance is not None:
                            if val.remote_field.null is False:
                                raise SDJExtrasError(
                                    f"There is a {key} already assigned to this {val.remote_field.name}. Maybe specify delete to replace the {key} ?")
                            else:  # noqa: RET506
                                rel["before"].append({"operation": "skip", "removals": [
                                    {"model": val.related_model, "rel_data_id": val.remote_field.name,
                                     "pks": [existing_instance.pk]}]})
            ...
            if isinstance(val, ForeignKey):
                ...
                if isinstance(_rel_input, CRUDOneToManyCreateInput):
                    ...
                    if _rel_input.update is not UNSET:
                        ...
                        if rel_obj is None:
                            raise SDJExtrasError("Cannot update non existing object for key %s" % key)
                ...
                if isinstance(_rel_input, CRUDManyToOneCreateInput):
                    ...
                    if _rel_input.assign is not UNSET:
                        ...
                        if rel_objs.count() != len(_rel_input.assign):
                            raise SDJExtrasError("Not all assigned objects found for %s" % val.related_model.__name__)
                    ...
                    if _rel_input.remove is not UNSET:
                        ...
                        if len(rem_pks) > 0 and val.remote_field.null is not True:
                            raise SDJExtrasError(
                                f"Cannot remove remote objects from non nullable field ( rel: {key} , model: {val.related_model.__name__})")
                        manager = getattr(val.model.objects.get(pk=int(_input.id)), val.get_accessor_name())
                        if len(del_pks) > 0:
                ...
                if isinstance(_rel_input, CRUDManyToManyCreateInput):
                    if isinstance(val, ManyToManyField):
                        ...
                        raise SDJExtrasError("Unable to determine manager for %s" % val.__class__.__name__)
                    if _rel_input.create is UNSET and _rel_input.assign is UNSET:
                        raise SDJExtrasError("Must create or assign or both")
                    if _rel_input.create is not UNSET:
...
def perform_validation(_input, info):
    ...
    if hasattr(_input, "__strawberry_definition__") and _input.__strawberry_definition__.is_input is True:
        ...
        for key, val in _input.__dict__.items():
            if val is not None and val is not UNSET and hasattr(_input, f"validate_{to_camel_case(key)}") and callable(
...
def check_permissions(_input, info):
    ...
    if hasattr(_input, "__strawberry_definition__") and _input.__strawberry_definition__.is_input is True:
        ...
        for key, val in _input.__dict__.items():
            if val is not None and val is not UNSET and hasattr(_input, f"check_permissions_{to_camel_case(key)}") and callable(
